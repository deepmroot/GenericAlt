...
export const Navigation: React.FC = () => {
  ...
  return (
    <header className="fixed top-6 left-0 right-0 z-50 px-4 md:px-8 pointer-events-none">
      <div className={`max-w-7xl mx-auto bg-[#0c0f01]/80 backdrop-blur-xl rounded-full border border-white/10 flex items-center justify-between p-2 pl-8 transition-all duration-500 pointer-events-auto ${isScrolled ? 'bg-[#0c0f01]/90' : 'bg-[#0c0f01]/60'}`}>
      ...
      {/* Mobile Nav Dropdown */}
      {mobileOpen && (
        <div className="md:hidden absolute top-full mt-4 left-4 right-4 bg-[#1a1c14] border border-[#d3f54c]/20 rounded-[2rem] p-6 flex flex-col gap-4 pointer-events-auto">
          {navLinks.map((link) => (
            <a 
              key={link.name}
              href={link.href} 
